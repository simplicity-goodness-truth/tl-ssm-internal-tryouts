...
*&
*&---------------------------------------------------------------------*
report zlukoil_sm3_link_partner_imp.
...
*
    ...
    " data lt_link_with_key type table of crmd_link with non-unique sorted key sort_key components guid_set.
    ...
    data:
      ...
      lt_link_with_key type table of ty_link with unique sorted key sort_key components guid_set.
    ...
    "sort lt_link_with_key.

    loop at lt_link_with_key into data(ls_link).
      lrs_link-sign = 'I'.
      lrs_link-option = 'EQ'.
    ...
    " append lines of lt_link to lt_link_with_key.


    data lt_partner_db    type table of crmt_object_guid .

    select guid  from crmd_partner into table lt_partner_db
      where guid in lr_link
      and partner_no = lv_partner_guid.


    if lt_partner_db is initial.
      do 1415 times.

        data(wa_partner_db) = cl_system_uuid=>create_uuid_x16_static( ).

        append wa_partner_db to lt_partner_db.
...
*        others        = 0.
    ...
    loop at lt_partner_db into data(ls_partner_db) .

      read table lt_link_with_key into ls_link_with_key with table key sort_key components guid_set = ls_partner_db.

      if ls_link_with_key-guid_hi is not initial.
        insert ls_link_with_key-guid_hi into table assigned_guids.
      endif.
    ...
    write: |lt_link_with_key records = | && |{ lines( lt_link_with_key ) }|.
