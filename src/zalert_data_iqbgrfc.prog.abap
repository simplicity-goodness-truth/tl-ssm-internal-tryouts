...
data: lr_inbound_destination    type ref to if_bgrfc_destination_inbound,
      ...
      lv_lines_sent_to_bgrfc    type integer,
...
do lines( lt_alerts_for_task ) times.
  ...
  lv_inbound_queue_name = |ZQTBALDATA_| && |{ sy-index }|.
  ...
  call function 'ZBGRFC_ALERT_DATA_READ' in background unit lr_inbound_qrfc_unit
    exporting
      it_alerts_list = lt_alerts_list.
  "     it_alerts_list = lt_context2.


  new-line.
  write: |{ sy-index }| && | lines = | && |{ lines( lt_alerts_list ) }|.
  " write: |{ sy-index }| && | lines = | && |{ lines( lt_context2 ) }|.

  lv_lines_sent_to_bgrfc = lv_lines_sent_to_bgrfc +  lines( lt_alerts_list ).
...
write: |lines sent to bgRFC = | && |{ lv_lines_sent_to_bgrfc }|.
...
do.
  ...
  select count( * ) into lv_zbgrfc_alrt_data_count from zbgrfc_alrt_data.

  if lv_lines_sent_to_bgrfc eq lv_zbgrfc_alrt_data_count.


    new-line.

    write: |lines in db table = | && |{ lv_zbgrfc_alrt_data_count }|.
